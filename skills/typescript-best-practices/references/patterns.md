...
Code examples for each rule in `SKILL.md`. Model valid states directly and validate external data once at the boundary.
...
Validate once where data crosses in. Trust types inside. Use the repository's schema library as described in [Schemas before hand-rolled guards](#schemas-before-hand-rolled-guards).
