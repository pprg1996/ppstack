...
description: "Audit a project's verification skill and feature map against source and live behavior. Use for 'maintain verification skill', 'audit the verify skill', or when a project-local verification harness may have drifted."
---
...
0. **Locate the target.** Find the project-local verification skill whose body has launch/drive sections and a feature map. Search the skill directories supported by the active environment instead of assuming one harness path. If several candidates exist, ask which one. If none exists, stop and point at `create-verification-skill` instead of inventing a target.
...
2. **Source wave.** Read every feature file against source. When isolated workers are available, assign one read-only worker per feature and launch them concurrently. Otherwise inspect features sequentially. Each review explains how the user-facing feature works, flags likely drift with citations, and returns one concise live-verification recipe. Workers never drive the app or edit files. Return shape: feature summary / source entry points / likely drift or none / one recipe.
